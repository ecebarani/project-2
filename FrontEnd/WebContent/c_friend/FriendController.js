myApp.controller("FriendController",function($scope,$rootScope,$http,$location)
{
	...
	function showFriendList()
	{
		...
		$http.get('http://localhost:8054/ChatMiddleware/showFriendList')
	...
	function showPendingFriendList()
	{
		...
		$http.get('http://localhost:8054/ChatMiddleware/showPendingFriendRequest')
		.then(function(response)
				{
	...
	function showSuggestedFriendList()
	{
		...
		$http.get('http://localhost:8054/ChatMiddleware/showSuggestedFriend')
		.then(function(response)
				{
	...
	$scope.sendFriendRequest=function(loginname)
	{
		...
		$http.post('http://localhost:8054/ChatMiddleware/sendFriendRequest',$scope.friend)
		.then(function(response)
				{
					console.log('Friend');
	...
	$scope.deleteFriend=function(friendId)
	{
		...
		$http.get('http://localhost:8054/ChatMiddleware/deleteFriendRequest/'+friendId)
		.then(function(response)
				{
				console.log(response.statusText);
	...
	$scope.acceptFriend=function(friendId)
	{
		...
		$http.get('http://localhost:8054/ChatMiddleware/acceptFriendRequest/'+friendId)
		.then(function(response)
				{
				console.log(response.statusText);
				$location.path("/showPendingFriend");
